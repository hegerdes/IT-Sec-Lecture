...
def socks_handler(context):
    ...
    print(struct.unpack('!HBBBB', data[2:8]))
    vn, cn = struct.unpack('!BB', data[:2])
    assert vn == CONST.SOCKS_VERSION

    port, ip1, ip2, ip3, ip4 = struct.unpack('!HBBBB', data[2:8])

    if cn != 1:
        # Bind is handeld by the SocketServer TCP
        print(CL.RED + 'Unsuported SOCKS cn')
    ...
    print((CL.BLU + '{} requestet {}:{}' + CL.NC).format(context.request.getpeername(), address, port) )
    ...
    try:
        ...
        print(CL.RED + 'Error in the dst connection!' + CL.NC + '\nErrMSG: ' +str(e))
